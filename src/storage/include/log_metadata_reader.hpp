...
public:
  ...
  std::vector<RecordBatchReader::RecordBatch> readPartitionLog(const std::string &topic_name,
...
private:
  ...
  std::vector<RecordBatchReader::RecordBatch> readAllBatches(std::ifstream &file);

  std::string getPartitionLogPath(const std::string &topic_name, int32_t partition_id);
};
